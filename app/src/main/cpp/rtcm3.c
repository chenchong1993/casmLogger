...
/* decode type 1104 message header --------------------------------------*/
static int decode_head1104(rtcm_t *rtcm, int *sync)
{
    double tow;
    char *msg;
    int i=24,staid,nsat,type,bdflag[3];
    if(rtcm->exlive&0x04){
        i += 16*8;
        //rtcm->len -= 16;
    }
    ...
    if (i+52<=rtcm->len*8) {
        ...
        tow  =getbitu(rtcm->buff,i,30)*0.001; i+=30;
        *sync=getbitu(rtcm->buff,i, 1);       i+= 1;
        nsat =getbitu(rtcm->buff,i, 5);       i+=9;
        bdflag[0]=getbitu(rtcm->buff,i, 1);  i+=1;
        bdflag[1]=getbitu(rtcm->buff,i, 1);  i+=1;
        bdflag[2]=getbitu(rtcm->buff,i, 1);  i+=1;
    }
    else {
        //TRACE("rtcm3 %d length error: len=%d\n",type,rtcm->len);
        return -1;
    }
    /* test station id */
    if (!test_staid(rtcm,staid)) return -1;
    //for (i=0;i<3;i++)
    //{
    //	rtcm->bdflag[i]=bdflag[i];
    //}
    adjweek_bds(rtcm,tow);
    rtcm->time=bdt2gpst(rtcm->time);
    //TRACE("decode_head1104: time=%s nsat=%d sync=%d\n",time_str(rtcm->time,2),
    ///	nsat,*sync);
...
/* decode type 1104: extended L1&L2 bds rtk observables ----------------------*/
static	int decode_type1104(rtcm_t *rtcm)
{
    const int L2codes[]={CODE_L2X,CODE_L7I,CODE_L7Q,CODE_L7X,CODE_L6X,CODE_L2I,CODE_L2Q,CODE_L6I,CODE_L6Q};
    double pr1,pr2,pr3,cnr1,cnr2,cnr3,tt,cp1,cp2,cp3;
    int i=24+67,j,index,nsat,sync,prn,sat,code1,code2,code3,ppr1,ppr2,ppr3;
    int lock1,lock2,lock3,amb1,amb2,amb3,sys;
    if(rtcm->exlive&0x04){
        i += 16*8;
        //rtcm->len -= 16;
    }
    if ((nsat=decode_head1104(rtcm,&sync))<0) return -1;

    for (j=0;j<nsat&&rtcm->obs.n<MAXOBS&&i+74<=rtcm->len*8;j++) {
        prn  =getbitu(rtcm->buff,i, 6); i+= 6;
        code1=getbitu(rtcm->buff,i, 2); i+= 2;
        pr1  =getbitu(rtcm->buff,i,24); i+=24;
        ppr1 =getbits(rtcm->buff,i,20); i+=20;
        lock1=getbitu(rtcm->buff,i, 7); i+= 7;
        amb1  =getbitu(rtcm->buff,i, 8); i+= 8;
        cnr1 =getbitu(rtcm->buff,i, 8); i+= 8;
        code2=getbitu(rtcm->buff,i, 2); i+= 2;
        pr2  =getbitu(rtcm->buff,i,24); i+=24;
        ppr2 =getbits(rtcm->buff,i,20); i+=20;
        lock2=getbitu(rtcm->buff,i, 7); i+= 7;
        amb2  =getbitu(rtcm->buff,i, 8); i+= 8;
        cnr2 =getbitu(rtcm->buff,i, 8); i+= 8;

        //code3=getbitu(rtcm->buff,i, 2); i+= 2;
        //pr3  =getbitu(rtcm->buff,i,24); i+=24;
        //ppr3 =getbits(rtcm->buff,i,20); i+=20;
        //lock3=getbitu(rtcm->buff,i, 7); i+= 7;
        //amb3  =getbitu(rtcm->buff,i, 8); i+= 8;
        //cnr3 =getbitu(rtcm->buff,i, 8); i+= 8;

        sys=SYS_CMP;
        if (!(sat=satno(sys,prn))) {
            //TRACE("rtcm3 1104 satellite number error: sys=%d prn=%d\n",sys,prn);
            continue;
        }
        ...
        if (ppr1!=(int)0xFFF80000) {
            rtcm->obs.data[index].P[0]=pr1;
            cp1=adjcp(rtcm,sat,0,ppr1*0.0005/(CLIGHT/FREQ2_CMP));
            rtcm->obs.data[index].L[0]=pr1/(CLIGHT/FREQ2_CMP)+cp1;
        }
        rtcm->obs.data[index].LLI[0]=lossoflock(rtcm,sat,0,lock1);
        rtcm->obs.data[index].SNR[0]=snratio(cnr1*0.25);
        rtcm->obs.data[index].code[0]=40;

        pr2=pr2*0.02+amb2*PRUNIT_GPS;
        if (pr2!=(int)0xFFFFE000) {
            rtcm->obs.data[index].P[1]=pr2;
            cp2=adjcp(rtcm,sat,0,ppr2*0.0005/(CLIGHT/FREQ1_CMP));
            rtcm->obs.data[index].L[1]=pr2/(CLIGHT/FREQ1_CMP)+cp2;
        }
        rtcm->obs.data[index].LLI[1]=lossoflock(rtcm,sat,1,lock2);
        rtcm->obs.data[index].SNR[1]=snratio(cnr2*0.25);
        rtcm->obs.data[index].code[1]=27;

        pr3=pr3*0.02+amb3*PRUNIT_GPS;
        if (pr3!=(int)0xFFFFE000) {
        	rtcm->obs.data[index].P[2]=pr3;
        	cp3=adjcp(rtcm,sat,0,ppr3*0.0005/(CLIGHT/FREQ3_CMP));
        	rtcm->obs.data[index].L[2]=pr3/(CLIGHT/FREQ3_CMP)+cp3;
        }
        rtcm->obs.data[index].LLI[2]=lossoflock(rtcm,sat,1,lock3);
        rtcm->obs.data[index].SNR[2]=snratio(cnr3*0.25);
        rtcm->obs.data[index].code[2]=42;
    }
    rtcm->obsflag=!sync;
    return sync?0:1;
}
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case 1104: ret=decode_type1104(rtcm); break;    //华辰BD观测值
    }
    if (ret>=0) {
        type-=1000;
