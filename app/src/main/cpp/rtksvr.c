...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<1;i++){
          ...
          /*
            //这里加手机观测值*/
            for(j = 0,i=0;i<32&&j<32&&obs.n<MAXOBS*2;i++,j++) {
//                if(ObsData.BDSTweek[j]!=0&& ObsData.BdsC1_t[j]!=0&&j<ObsData.bdsSvNum){
...
//                    obs.data[obs.n].sat = ObsData.Bdsprn_t[j];
//                    obs.data[obs.n].sat = (unsigned char)satno(SYS_CMP,ObsData.Bdsprn_t[j]);
//                    obs.data[obs.n].rcv =  '\x01';
//                    obs.n++;
//                }
                if (ObsData.GPSTweek[i]!=0&& ObsData.GpsC1_t[i]!=0&&i<ObsData.gpsSvNum){
                    gtime_t gpstTemp = gpst2time(ObsData.GPSTweek[i],ObsData.GPSTsecond[i]);
                    obs.data[obs.n].time.time = gpstTemp.time;
                    obs.data[obs.n].time.sec = gpstTemp.sec;
                    obs.data[obs.n].P[0] = ObsData.GpsC1_t[i];
                    obs.data[obs.n].L[0] = ObsData.GpsC1_t[i]/0.19029;
                    obs.data[obs.n].D[0] = ObsData.GpsD1_t[i];
                    obs.data[obs.n].sat = ObsData.Gpsprn_t[i];
                    obs.data[obs.n].rcv = '\x01';
                    obs.data[obs.n].sat = (unsigned char)satno(SYS_GPS,ObsData.Gpsprn_t[i]);//GPS卫星号
                    obs.n++;
                }
...
//                obs.data[obs.n].L[0] = ObsData.GpsL1_t[j];
//                obs.data[obs.n].D[0] = ObsData.GpsD1_t[j];
//                obs.data[obs.n].sat = ObsData.Gpsprn_t[j];
//                obs.data[obs.n].rcv = '\x01';
//                obs.data[obs.n].sat = (unsigned char)satno(SYS_GPS,ObsData.Gpsprn_t[j]);//GPS卫星号
//                obs.n++;
//            }
...
//                obs.data[obs.n].P[1] = ObsData.GloC2_t[j];
//                obs.data[obs.n].L[1] = ObsData.GloL2_t[j];
////                obs.data[obs.n].D[1] = ObsData.GloD2_t[j];
//                obs.data[obs.n].P[2] = ObsData.GloC3_t[j];
//                obs.data[obs.n].L[2] = ObsData.GloL3_t[j];
////                obs.data[obs.n].D[2] = ObsData.GloD3_t[j];
//                obs.data[obs.n].rcv =  (unsigned char)1;
//                //int2char(&obs.data[obs.n].sat,satno(SYS_GLO,ObsData.Gloprn_t[j]));
//                obs.n++;
//            }
...
//                obs.data[obs.n].P[1] = ObsData.GalC2_t[j];
//                obs.data[obs.n].L[1] = ObsData.GalL2_t[j];
////                obs.data[obs.n].D[1] = ObsData.GalD2_t[j];
//                obs.data[obs.n].P[2] = ObsData.GalC3_t[j];
//                obs.data[obs.n].L[2] = ObsData.GalL3_t[j];
////                obs.data[obs.n].D[2] = ObsData.GalD3_t[j];
//                obs.data[obs.n].rcv = (unsigned char)1;
//                //int2char(&obs.data[obs.n].sat,satno(SYS_GAL,ObsData.Galprn_t[j]));
//                obs.n++;
//            }
