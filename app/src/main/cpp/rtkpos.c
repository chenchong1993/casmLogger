                 ...
                 double *e, double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        /* undifferenced phase/code residual for satellite 卫星残差*/
...
static int validobs(int i, int j, int f, int nf, double *y)
{
    ...
    return y[f+i*nf*2]!=0.0&&y[f+j*nf*2]!=0.0&&
           (f<nf||(y[f-nf+i*nf*2]!=0.0&&y[f-nf+j*nf*2]!=0.0));  //0
}
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (i=-1,j=0;j<ns;j++) {
            ...
            if (!validobs(iu[j],ir[j],f,nf,y)) continue;//y的值有问题
                  ...
                  const nav_t *nav)
{
    ...
    /* select common satellites between rover and base-station 共视卫星*/
    ...
    /*edit by liuyi*/
    rtk->sol.satnum[0] = 0; rtk->sol.satnum[1] = 0;
    rtk->sol.satnum[2] = 0; rtk->sol.satnum[3] = 0;
    rtk->sol.satnum[4] = 0; rtk->sol.satnum[5] = 0;
    for(i = 0;i< nu;i++){
        sysprn = 0;
