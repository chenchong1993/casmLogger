...
public class UiLogger implements GnssListener {
    ...
    GnssRawData GnssRawDataArray[] = new GnssRawData[MaxSatelliteIndex];
...
//-----------------------------把array赋给全局变量，方便传递到JNI里面--------------------------------------------------------------------------------
        int DataLength = 0;
        double tRxSeconds;
        double weekNumber = Math.floor(-(event.getClock().getFullBiasNanos() * 1e-9 / 604800));
        double weekNumberNanos = weekNumber * 604800 * 1e9;
        if (event.getClock().hasBiasNanos() == false) {
            tRxSeconds = (event.getClock().getTimeNanos() - event.getClock().getFullBiasNanos() - weekNumberNanos) * 1e-9;
        } else {
            tRxSeconds = (event.getClock().getTimeNanos() - event.getClock().getFullBiasNanos() - event.getClock().getBiasNanos() - weekNumberNanos) * 1e-9;
        }

        for (int i = 0; i < array.length; i++) {
            if (array[i][0] != null
                    && array[i][1] != "INVALID_VALUE"
                    && Double.parseDouble(array[i][1].substring(0, 10)) > 2000000
                    && array[i][1].indexOf("FIX") != 0
                    && Double.parseDouble(array[i][3])>25.0) {
                ...
                StaticGnssData.GPSTsecond[DataLength] = tRxSeconds;
                DataLength++;
            }
        }
